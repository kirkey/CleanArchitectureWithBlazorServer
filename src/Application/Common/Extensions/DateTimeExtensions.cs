public static class DateTimeExtensions
{
    ...
    /// </example>
    public static (DateTime Start, DateTime End) GetDateRange(this DateTime dateTime, string keyword,
        TimeSpan timeZoneOffset = default)
    {
        var today = dateTime.Date;

        // Adjust for timezone offset if the DateTime is in UTC
        if (dateTime.Kind == DateTimeKind.Utc && timeZoneOffset != default) today = today.Subtract(timeZoneOffset);
        ...
        switch (keyword.ToUpper())
        {
            ...
            case "LAST_WEEK":
                return (today.AddDays(-7).StartOfWeek(timeZoneOffset),
                    today.AddDays(-7).EndOfWeek(timeZoneOffset).AddTicks(-1));
            case "NEXT_WEEK":
                return (today.AddDays(7).StartOfWeek(timeZoneOffset),
                    today.AddDays(7).EndOfWeek(timeZoneOffset).AddTicks(-1));
            case "THIS_MONTH":
            ...
            case "LAST_MONTH":
                ...
                return (new DateTime(lastMonth.Year, lastMonth.Month, 1),
                    lastMonth.EndOfMonth(timeZoneOffset).AddTicks(-1));
            case "NEXT_MONTH":
                var nextMonth = today.AddMonths(1);
                return (new DateTime(nextMonth.Year, nextMonth.Month, 1),
                    nextMonth.EndOfMonth(timeZoneOffset).AddTicks(-1));
            case "THIS_YEAR":
            ...
            case var kw when kw.StartsWith("LAST_") && kw.EndsWith("_DAYS"):
                if (int.TryParse(kw.Split('_')[1], out var days)) return (today.AddDays(-days), today.AddTicks(-1));
                break;
            case var kw when kw.StartsWith("NEXT_") && kw.EndsWith("_DAYS"):
                if (int.TryParse(kw.Split('_')[1], out var nextDays))
                    return (today, today.AddDays(nextDays).AddTicks(-1));
                break;
            case var kw when kw.StartsWith("LAST_") && kw.EndsWith("_MONTHS"):
                if (int.TryParse(kw.Split('_')[1], out var months))
                {
            ...
            case var kw when kw.StartsWith("NEXT_") && kw.EndsWith("_MONTHS"):
                if (int.TryParse(kw.Split('_')[1], out var nextMonths))
                {
    ...
    {
        var endOfWeek = dt.StartOfWeek(timeZoneOffset).AddDays(6);
        if (timeZoneOffset != default && dt.Kind == DateTimeKind.Utc) endOfWeek = endOfWeek.Subtract(timeZoneOffset);
    ...
    public static DateTime EndOfMonth(this DateTime dt, TimeSpan timeZoneOffset = default)
    {
        ...
        if (timeZoneOffset != default && dt.Kind == DateTimeKind.Utc) endOfMonth = endOfMonth.Subtract(timeZoneOffset);
    ...
    {
        var startOfMonth = new DateTime(dt.Year, dt.Month, 1);
        if (timeZoneOffset != default && dt.Kind == DateTimeKind.Utc)
    ...
    public static string? ToLocalTime(this DateTime? utcTime, TimeSpan? localTimeOffset,
