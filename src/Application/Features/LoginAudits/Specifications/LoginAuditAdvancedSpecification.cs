...
public class LoginAuditAdvancedSpecification : Specification<LoginAudit>
{
    public LoginAuditAdvancedSpecification(LoginAuditAdvancedFilter filter)
    {
        ...
        var last30daysrange = today.GetDateRange(LoginAuditListView.Last30Days.ToString(),filter.CurrentUser.LocalTimeOffset);
        ...
        Query.Where(p => p.LoginTimeUtc != default, filter.Keyword is null)
             ...
             .Where(x => x.LoginTimeUtc >= last30daysrange.Start, filter.ListView == LoginAuditListView.Last30Days);
