...
public class MinioUploadService : IUploadService
{
    ...
    public async Task<string> UploadAsync(UploadRequest request)
    {
        ...
        if (!bucketExists) await _minioClient.MakeBucketAsync(new MakeBucketArgs().WithBucket(_bucketName));

        // Build folder path based on UploadType and optional Folder property.
        var folderPath = $"{request.UploadType.GetDescription()}";
        if (!string.IsNullOrWhiteSpace(request.Folder)) folderPath = $"{folderPath}/{request.Folder.Trim('/')}";

        // Construct the object name including the folder path.
        var objectName = $"{folderPath}/{request.FileName}";
    ...
    public async Task RemoveAsync(string filename)
    {
        ...
        var pathParts = fileUri.AbsolutePath.TrimStart('/').Split('/', 2);
