...
public class IdentityService : IIdentityService
{
    ...
    public string GetUserName(string userId)
    {
        ...
        var user = _fusionCache.GetOrSet(key, _ => _userManager.Users.SingleOrDefault(u => u.Id == userId),
            RefreshInterval);
    ...
    public async Task<List<ApplicationUserDto>?> GetUsers(string? tenantId, CancellationToken cancellation = default)
    {
        ...
        var result =
            await _fusionCache.GetOrSetAsync(key, _ => getUsersByTenantId(tenantId, cancellation), RefreshInterval);
        return result;
    }
