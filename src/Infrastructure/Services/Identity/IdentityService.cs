    ...
    : IIdentityService, IDisposable
{
    ...
    public async Task<string?> GetUserNameAsync(string userId, CancellationToken cancellation = default)
    {
        ...
        var user = await fusionCache.GetOrSetAsync(key,
             async _ =>
             {
    ...
    public string GetUserName(string userId)
    {
        ...
        var user = fusionCache.GetOrSet(key, _ =>
        {
            using var scope = scopeFactory.CreateScope();
            var userManager = scope.ServiceProvider.GetRequiredService<UserManager<ApplicationUser>>();
        ...
        CancellationToken cancellation = default)
    {
        ...
        var result = await fusionCache.GetOrSetAsync(key,
            async _ =>
            {
                using var scope = scopeFactory.CreateScope();
                var userManager = scope.ServiceProvider.GetRequiredService<UserManager<ApplicationUser>>();
        ...
        Func<string?, CancellationToken, Task<List<ApplicationUserDto>?>> getUsersByTenantId =
            async (tenantId, token) =>
            {
                ...
                if (string.IsNullOrEmpty(tenantId))
                {
                    ...
                    .ProjectTo<ApplicationUserDto>(mapper.ConfigurationProvider).ToListAsync(token);
                }
                return await userManager.Users.Where(x => x.TenantId == tenantId).Include(x => x.UserRoles)
                    .ThenInclude(x => x.Role)
                    .ProjectTo<ApplicationUserDto>(mapper.ConfigurationProvider).ToListAsync(token);
            };
        var result = await fusionCache.GetOrSetAsync(key, _=>getUsersByTenantId(tenantId, cancellation), RefreshInterval);
        return result;
    }

    public void RemoveApplicationUserCache(string userName)
    {
        fusionCache.Remove(GetApplicationUserCacheKey(userName));
    }
