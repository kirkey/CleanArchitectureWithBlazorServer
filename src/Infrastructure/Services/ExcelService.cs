...
public class ExcelService(IStringLocalizer<ExcelService> localizer) : IExcelService
{
    ...
    public Task<byte[]> ExportAsync<TData>(IEnumerable<TData> data, Dictionary<string, Func<TData, object?>> mappers,
        ...
        string sheetName = "Sheet1")
    {
        ...
        for (var rowIndex = startRow; rowIndex <= lastRowNumber; rowIndex++)
        {
            ...
            try
            {
                ...
                {
                    var colIndex = cell.Address.ColumnNumber - 1;
                ...
                foreach (var header in headers) mappers[header](dataRow, item);
                list.Add(item);
            }
            catch (Exception e)
            {
